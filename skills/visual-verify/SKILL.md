...
Where a state cannot be reached from the UI, drive it by intercepting the request. A state you could not reach is recorded as unverified, with what blocked it.
...
Every state in the spec has a screenshot or an explicit unverified entry with its blocker, console and network are recorded for each state including the negatives, the four accessibility checks have been run, and every defect found is either fixed and re-verified or written into the handover.
